...
- **Mac system output volume** — controls the macOS output volume (not the speaker's BLE volume)
...
`install.sh` does everything for you (no sudo required, idempotent):
...
installed, it tells you how to add it). If you want to distribute a pre-built app, run
`bash package.sh` to produce a self-contained `.app` plus a zip with an auto-installer.

This repository is the **source**. Edit the code and re-run `bash install.sh` to apply your changes.
...
`volume_up=8002`, `volume_down=8003`, `play_pause=8004`,
`input_bluetooth=8101`, `input_aux=8102`, `input_usb=8103`,
...
> GATT commands are present in the protocol but **not sent by the current UI**.
