    ...
    <div id="controls" style="opacity:1; pointer-events:auto; transition:opacity .3s;">
      ...
      <div style="margin-bottom:20px;">
        <div style="display:flex; justify-content:space-between; align-items:baseline; margin-bottom:10px;">
          <span style="font-size:11px; font-weight:700; letter-spacing:.1em; text-transform:uppercase; color:#94a08f;" data-i18n="volume">Volume</span>
          <span id="volVal" style="font-family:ui-monospace,monospace; font-size:14px; font-weight:700; color:#7FE0A3;">0</span>
        </div>
        <div id="volSlider" style="position:relative; height:22px; display:flex; align-items:center; cursor:pointer; touch-action:none;">
          <div style="position:absolute; left:0; right:0; height:6px; border-radius:3px; background:rgba(255,255,255,.08);"></div>
          <div id="volFill" style="position:absolute; left:0; height:6px; border-radius:3px; width:0%; background:linear-gradient(90deg,#2F9E5F,#7FE0A3); box-shadow:0 0 10px rgba(107,203,139,.5);"></div>
          <div id="volKnob" style="position:absolute; left:0%; transform:translateX(-50%); width:18px; height:18px; border-radius:50%; background:#CFF2DD; border:3px solid #2F9E5F; box-shadow:0 3px 8px rgba(0,0,0,.5);"></div>
        </div>
      </div>
...
<script>
  ...
  // ---- volume slider (drag) ----
  var dragging=false, lastSent=-1, sendTimer=null;
  var slider=$('volSlider');
  function pct(e){ var r=slider.getBoundingClientRect(); var p=(e.clientX-r.left)/r.width; return Math.max(0,Math.min(100,Math.round(p*100))); }
  function paintVol(v){ $('volVal').textContent=v; $('volFill').style.width=v+'%'; $('volKnob').style.left=v+'%'; }
  function pushVol(v){
    if(v===lastSent) return; lastSent=v;
    if(sendTimer) return;                     // 150ms スロットル
    sendTimer=setTimeout(function(){ sendTimer=null; }, 150);
    send('volume', v);
  }
  slider.addEventListener('pointerdown', function(e){
    dragging=true; slider.setPointerCapture(e.pointerId);
    var v=pct(e); paintVol(v); pushVol(v);
  });
  slider.addEventListener('pointermove', function(e){ if(dragging){ var v=pct(e); paintVol(v); pushVol(v); } });
  slider.addEventListener('pointerup', function(e){
    if(dragging){ dragging=false; try{ slider.releasePointerCapture(e.pointerId); }catch(_){}
      var v=pct(e); paintVol(v); lastSent=-1; send('volume', v); }
  });
  ...
  function applyState(s){
    ...
    // volume(ドラッグ中は上書きしない)
    if(!dragging && s.volume!==undefined && s.volume!==null) paintVol(s.volume);
