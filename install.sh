...
if [ -z "${PYTHON:-}" ] && ! command -v python3.12 >/dev/null 2>&1; then
    ...
    if command -v brew >/dev/null 2>&1; then
        ...
        echo "Homebrew が見つかりません。Homebrew をインストールして Python 3.12 を導入します。"
        echo ""
        echo "  ダウンロード元 (Homebrew 公式):"
        echo "    https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh"
        echo "  実行するコマンド:"
        echo '    /bin/bash -c "$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/HEAD/install.sh)"'
        echo "  インストール先: Apple Silicon では /opt/homebrew、Intel では /usr/local"
        echo ""
        # 非対話(CI 等)では確認できないため停止する
        if [ ! -t 0 ]; then
            echo "エラー: 非対話のため Homebrew を自動インストールしません。導入後に再実行してください:" >&2
            echo "  - Homebrew: https://brew.sh" >&2
            exit 1
        fi
        # set -e 下でも EOF 時に案内を表示できるよう、read を条件式内で扱う
        if ! read -r -p "このコマンドで Homebrew をインストールしますか? (y/N): " ans; then
            echo ""
            echo "入力を取得できませんでした。Homebrew を導入後に再実行してください:" >&2
            echo "  - Homebrew: https://brew.sh" >&2
            exit 1
        fi
        if [ "$ans" = "y" ] || [ "$ans" = "Y" ]; then
            ...
            if ! command -v brew >/dev/null 2>&1; then
                echo "エラー: Homebrew のインストール後に brew を検出できませんでした。" >&2
                exit 1
            fi
            echo "==> Homebrew で python@3.12 を導入します"
            brew install python@3.12
            ...
            echo "Homebrew をインストールしませんでした。導入後に再実行してください:" >&2
            echo "  - Homebrew: https://brew.sh" >&2
            exit 1
        fi
    fi
